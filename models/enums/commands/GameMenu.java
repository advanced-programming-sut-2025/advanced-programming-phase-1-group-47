...
public enum GameMenu {
    ...
    inventorytrashtotal("^\\s*inventory\\s+trash\\s+-i\\s+(?<itemName>.+?)\\s+-n\\s+(?<number>\\d+)\\s*$"),
    inventorytrash("^\\s*inventory\\s+trash\\s+-i\\s+(?<itemName>.+?)\\s*$"),
    ...
    cookingrefrigeratorput("^\\s*cooking\\s+refrigerator\\s+put\\s+(?<item>.+?)\\s*$"),
    cookingrefrigeratorpick("^\\s*cooking\\s+refrigerator\\s+pick\\s+(?<item>.+?)\\s*$"),
    cookingreciepe("^\\s*cooking\\s+show\\s+recipes\\s*$"),
    cookingprepare("^\\s*cooking\\s+prepare\\s+(?<recipeName>.+?)\\s*$"),
    eatfood("^\\s*eat\\s+(?<foodName>.+?)\\s*$"),
    ...
    sellproduct("^\\s*sell\\s+(?<productName>.+?)\\s+-n\\s+(?<count>\\d+)\\s*$"),
