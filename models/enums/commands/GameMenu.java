...
public enum GameMenu {
    ...
    cheatadvencedate("^\\s*cheat\\s+advance\\s+date\\s+(?<date>\\d{4}-\\d{2}-\\d{2})\\s*$"),
    ...
    inventorytrashtotal("^\\s*inventory\\s+trash\\s+-i\\s+(?<item_name>\\S+)\\s+-n\\s+(?<number>\\d+)\\s*$"),
    inventorytrash("^\\s*inventory\\s+trash\\s+-i\\s+(?<item_name>\\S+)\\s*$"),
    ...
    craftinfo("^\\s*craftinfo\\s+-n\\s+(?<craft_name>\\S+)\\s*$"),
    ...
    cheatadditem("^\\s*cheat\\s+add\\s+item\\s+-n\\s+(?<item_name>\\S+)\\s+-c\\s+(?<count>\\d+)\\s*$"),
    ...
    cheatsetanimalfriendship("^\\s*cheat\\s+set\\s+friendship\\s+-n\\s+(?<animal_name>\\S+)\\s+-c\\s+(?<amount>\\d+)\\s*$"),
    showanimals("^\\s*animals\\s*$"),
    shepherdanimals("^\\s*shepherd\\s+animals\\s+-n\\s+(?<animal_name>\\S+)\\s+-l\\s+(?<x>\\d+),(?<y>\\d+)\\s*$"),
    feedhay("^\\s*feed\\s+hay\\s+-n\\s+(?<animal_name>\\S+)\\s*$"),
    ...
    purchaseproduct("^\\s*purchase\\s+(?<product_name>\\S+)\\s+-n\\s+(?<count>\\d+)\\s*$"),
    purchaseoneproduct("^\\s*purchase\\s+(?<product_name>\\S+)\\s*$"),
    cheatadddollars("^\\s*cheat\\s+add\\s+(?<count>\\d+)\\s+dollars\\s*$"),
    sellproduct("^\\s*sell\\s+(?<product_name>\\S+)\\s+-n\\s+(?<count>\\d+)\\s*$"),
    ...
    giftrate("^\\s*gift\\s+rate\\s+-i\\s+(?<gift_number>\\d+)\\s+-r\\s+(?<rate>\\d+)\\s*$"),
